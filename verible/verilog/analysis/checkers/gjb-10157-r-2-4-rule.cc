...
VERILOG_REGISTER_LINT_RULE(Gjb10157R24Rule);

namespace {

// Forbidden power supply names (lowercase for comparison)
const std::set<std::string> kForbiddenPowerNames = {
    "vdd", "vss", "vcc", "gnd", "vref"
};

}  // namespace

const LintRuleDescriptor &Gjb10157R24Rule::GetDescriptor() {
  static const LintRuleDescriptor d{
      .name = "GJB-10157-R-2-4",
      .topic = "identifiers",
      .desc =
          "Checks that identifiers do not use power supply names: "
          "VDD, VSS, VCC, GND, or VREF (case-insensitive). "
          "[GJB 10157 Rule 2-4]",
  };
  return d;
}

absl::Status Gjb10157R24Rule::Configure(std::string_view configuration) {
  if (configuration.empty()) return absl::OkStatus();
  return absl::InvalidArgumentError(
      "This rule does not accept any configuration.");
}

bool Gjb10157R24Rule::IsForbiddenPowerName(std::string_view name) {
  std::string lower_name = absl::AsciiStrToLower(name);
  return kForbiddenPowerNames.count(lower_name) > 0;
}

void Gjb10157R24Rule::Lint(const verible::TextStructureView &text_structure,
                           std::string_view filename) {
  const auto &tree = text_structure.SyntaxTree();
  if (tree == nullptr) return;

  // Helper lambda to check identifier and add violation if forbidden
  auto check_identifier = [&](const verible::TokenInfo &token,
                              const verible::SyntaxTreeContext &context,
                              std::string_view kind) {
    std::string_view name = token.text();
    if (IsForbiddenPowerName(name)) {
      std::string reason = absl::StrCat(
          kind, " name '", name,
          "' is a forbidden power supply name (VDD/VSS/VCC/GND/VREF). "
          "[GJB 10157 R-2-4]");
...
verible::LintRuleStatus Gjb10157R24Rule::Report() const {
